...
CREATE TABLE `siswa` (
  ...
  `access_token` CHAR(32)       NULL COMMENT 'Token acak (hex) untuk link rapor publik /rapor/{token}, dibaca orangtua TANPA login. NULL sampai pertama kali di-generate oleh admin/guru. Regenerable kapan saja jika dianggap bocor.',
  `created_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`  DATETIME        NULL     ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`nisn`),
